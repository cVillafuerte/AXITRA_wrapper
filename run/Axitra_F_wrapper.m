% -------------------------------------------------------------------------
%
% Matlab Wrapper to execute AXITRA for the Force version
%
...
sources(4) = 1000; % Depth coordinate
...
F_ampl    = 1e15;     % Vector (ns,1) with the total force amplitude
fx        = 0;        % Vector (ns,1) with the normalized force in the East direction
fy        = 0;        % Vector (ns,1) with the normalized force in the North direction
fz        = 1.0;      % Vector (ns,1) with the normalized force in the Z direction
t0_delay  = 2;        % Vector (ns,1) with the time.delay of every source

source_opt  = 1;     % Source time function option. 
...
% If read it from files, it should be describe in 4 columns:
